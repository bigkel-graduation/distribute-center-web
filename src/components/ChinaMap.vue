...
import textAPI from "../request/userManageApi.js";
...
} from "../libs/geoNameDictionary.js"; // 引入省市对应英文名字
...
export default {
  ...
  methods: {
      ...
      if (
        ...
        if (this.placeType == "city" && this.province.name) {
          ...
          this.option.series[0].data = this.convertData(
            this.data,
            provinceNameChineseToEng(this.province.name)
          );
      ...
      this.echart.on("click", params => {
        ...
        textAPI.CakeAPI(params.name).then(res => {
          ...
          window.localStorage.setItem(
            "cake",
            JSON.stringify(res.data.data.city)
          );
        ...
        // 对点击的省或者市做不同的处理
        let province = provinceNameArr.find(
        ...
        if (this.placeType === "country" && province) {
          ...
          Object.keys(
            cityNameData[
              `cityName_${provinceNameChineseToEng(this.province.name)}`
            ]
          ).find(item => item === params.name)
        ) {
          // 点击了市，跳转到该市
    ...
    setProvinceMap(name, code) {
      axios
        .get(`/json/geometryProvince/${code}.json`)
        .then(res => {
          this.placeType = "province";
    ...
    setCityMap(name, code) {
      axios
        .get(`/json/geometryCouties/${code}.json`)
        .then(res => {
          this.placeType = "city";
          this.city.name = name;
          this.city.code = code;
